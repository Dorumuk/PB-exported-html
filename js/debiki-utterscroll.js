...
/*
    if ($target.css('overflow') == 'hidden') return;
        alert("abcd");
*/


    var $noScrollElem = $target.closest(allScrollstoppers);
    console.log($noScrollElem.length);
  ...
  function startScroll(event) {
    ...
    $(parent.document).mouseup(stopScroll);
    
    var x = parent.document.getElementById("contents");
    var y = (x.contentWindow || x.contentDocument);
     $(y.document).mouseup(stopScroll);
     
     var x2 = parent.document.getElementById("contents2");
    var y2 = (x2.contentWindow || x2.contentDocument);
     $(y2.document).mouseup(stopScroll);
